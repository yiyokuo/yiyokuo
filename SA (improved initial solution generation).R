...
#中4-14(14)
#relation = matrix(c(1,8,1,4,2,3,3,4,4,10,4,11,4,13,5,6,6,8,6,9,7,1,7,2,7,6,8,12,9,12,10,14,11,15,13,15,13,15,14,16,15,16), ncol = 2, byrow = T)
#T = c(3,2,4,2,3,5,4,5,4,3,5,4,10,7,4,7)

#中4-15(15)
#relation = matrix(c(1,3,1,4,2,5,3,6,4,7,5,7,6,8,7,8,7,9,7,10,8,11,9,12,9,13,10,13,11,14,11,15,12,15,13,16), ncol = 2, byrow = T)
#T = c(6,5,2,9,8,4,7,4,5,4,6,5,6,4,3,4)

#中4-16(16)
#relation = matrix(c(1,11,2,5,2,6,3,6,3,7,4,15,5,8,6,9,7,10,8,12,9,12,9,13,9,14,10,14,11,16,12,17,13,18,13,19,14,19,15,20,16,21,17,21,18,22,19,23,20,23,20,24), ncol = 2, byrow = T)
#T = c(3,7,7,5,4,3,4,3,6,4,4,3,3,9,5,9,2,7,9,9,8,8,9,9)
...
#函數：產生初始解
solGen = function(Wmax, PR, T, a, b, improve = FALSE, gamma=0.9){
  ...
  while (CT_Wmax < sum(T)/Wmax){
    ...
    while (k < Wmax){
      totaltime = 0
      while (totaltime < ACT && j_overbar != j_underbar){
        # #if (j_overbar != j_underbar){
        # if (max(T[D])+totaltime<ACT){
        #   i = sample(D,1)
        #   #print(i)
        #   Y[i] = k
        #   #print(Y)
        #   totaltime = totaltime + T[i]
        # } else if (max(T[D])+totaltime>ACT && min(T[D])+totaltime<ACT){
        #   i = D[which(T[D]==min(T[D]))]
        #   Y[i] = k
        #   totaltime = totaltime + T[i]
        # } else {
        #   break
    ...
    #output['Y',which(output['Y',]==0)]=Wmax
    CT_Wmax = sum(output['T',which(output['Y',]==Wmax)])
    #print(paste('totaltime', CT_Wmax))
    ACT = ACT*gamma
  }
...
plotU = function(output, a, b){
  ...
  points(c(output['Startx',],output['Endx',]), c(output['Starty',], output['Endy',]), type = 'p', lwd = 20, col='yellow')
}
...
while (h < H){
  ...
  tryCatch({
    ...
    #要執行的指令放這裡
    O = solGen(Wmax, PR, T, a, b, gamma=0.9)
...
sumT = {}
for (i in 1:Wmax){
  sumT = c(sumT, sum(Oglobal['T',which(Oglobal['Y',]==i)]))
}
print(sumT)
print(sum(T)/Wmax)
